...
## Data provenance and NAS100 4H recovery

Experiment 2 uses the existing 26-context universe (13 instruments x 2 timeframes).

Most reusable datasets come from the previously validated Nova/Dukascopy artifacts. WTI 1D uses the already-documented independent Yahoo Finance source because the native Dukascopy WTI daily history was not recoverable reliably.

NAS100 4H has a known historical source-availability gap in Dukascopy: the `USATECHIDXUSD` hourly feed does not provide the required 2010 through August 2011 period. To preserve the frozen 2010-2025 NAS100 context rather than silently dropping that history, the following source splice is frozen for Experiment 2 development and final evaluation:

- 2010-01-01 through 2011-08-31: FutureSharks/financial-data OANDA `NAS100_USD` minute archive.
- 2011-09-01 through 2025-12-31: Nova's reviewed Dukascopy-native NAS100 4H acquisition path.
- The early OANDA minute data is aggregated deterministically into UTC 4H OHLCV bars.
- The splice boundary is fixed at 2011-09-01 00:00 UTC.
- Duplicate timestamps at the boundary are rejected rather than silently selected.
- OHLC integrity and chronological ordering are required for both source segments and the combined series.
- A provenance record is written alongside the NAS100 dataset documenting the source boundary and aggregation method.

This is a data-recovery rule frozen before Experiment 2 development evaluation. It is not permitted to be changed after inspecting development or final-test results. A future alternative NAS100 data source would constitute a new experiment.
