...
def _bars(n=260):
    ...
    return [
        Bar(timestamp=base + timedelta(days=i), open=100+i*0.01, high=101+i*0.01, low=99+i*0.01, close=100+i*0.01, volume=1.0)
        for i in range(n)
    ]


def test_contextual_ensemble_schema_and_causality_contract():
    result = evaluate_contextual_online_expert_ensemble(_bars(), min_global_history=0, min_context_history=0)
