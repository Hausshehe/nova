...
15. Treat the user's complete request as the goal state, not just the next tap.
16. For multi-step requests, keep the entire requested destination chain in mind.
17. A successful tap is NOT evidence that the intended destination was reached.
    Re-observe and compare the resulting UI with the requested goal.
18. If the current screen does not meaningfully advance the goal, treat the
    previous choice as a possible mistake. Re-plan from the new state.
19. When a branch leads to an unrelated or dead-end screen, use back_android when
    appropriate and try a different semantically relevant control.
20. Never select a control solely because it shares one generic word with the
    target. Consider the complete wording and the current UI context.
21. Phrases such as "open X in Settings", "open X inside Settings", or
    "go to X in Settings" describe a destination within Settings; they do NOT
    mean launching X as a standalone application.
22. For chained requests, verify every intermediate destination before moving
    to the next one, and verify the final destination before claiming success.
...
def run_agent(goal):
    ...
    # Keep compound/natural-language navigation in the real planner.
    # The planner has the full goal, live observations, and recovery tools.
    # Only a standalone app launch gets the local fast path.
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if not tool_calls:
            ...
            # A planner's natural-language response is never proof that the
            # Android goal is complete. The controller must require a verified
            # post-action observation before returning success.
            if action_seen and not observed_after_action:
                messages.append({
                    "role": "system",
                    "content": (
                        "DO NOT claim the goal is complete. A state-changing "
                        "action occurred without a verified post-action "
                        "observation. Call observe_android now, inspect the live "
                        "UI, and continue planning toward the ORIGINAL USER GOAL."
                    ),
                })
                continue
            ...
            if not action_seen:
                messages.append({
                    "role": "system",
                    "content": (
                        "The original Android goal has not been demonstrated as "
                        "complete. Observe the current UI and use the generic "
                        "tools to accomplish and verify the user's goal. Do not "
                        "return success merely because you can describe a plan."
                    ),
                })
                continue
            ...
            messages.append({
                "role": "system",
                "content": (
                    "Before reporting success, verify the FINAL requested "
                    "destination against the newest Android observation. If the "
                    "screen is intermediate, unrelated, or a dead end, continue "
                    "or recover with generic actions. A tool selection or "
                    "successful tap is not goal completion."
                ),
            })
            continue
        ...
        # If an action succeeded but the resulting state is not obviously
        # consistent with the original goal, force an explicit re-evaluation.
        # This is generic recovery guidance; it does not name Android screens.
        if (
            function_name not in {"observe_android", "find_android_app"}
            and isinstance(result, dict)
            and result.get("success")
            and result.get("post_action_observation")
        ):
            messages.append({
                "role": "system",
                "content": (
                    "Re-evaluate the ORIGINAL USER GOAL against the newest "
                    "observation. Do not assume the last action was correct. "
                    "If the new screen is only an intermediate step, continue. "
                    "If it is an unrelated or dead-end branch, recover with "
                    "back_android or another generic action and choose a better "
                    "semantic route. Claim success only when the final requested "
                    "destination is visibly and reliably verified."
                ),
            })
