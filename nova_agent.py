...
def _planner_tool_result(result, function_name):
    ...
    if function_name == "observe_android" and result.get("success"):
        ...
        compact_state = {
            ...
            "interactive_labels": [node.get("label", "") for node in state.get("interactive", []) if node.get("label")],
            "scrollable": state.get("scrollable", []),
            "packages": state.get("packages", []),
            "node_count": state.get("node_count", result.get("node_count", 0)),
            "foreground_package": foreground_package,
        }
        return {"success": True, "verified": bool(result.get("verified")), "summary": result.get("summary", ""), "state": compact_state, "foreground_package": foreground_package}
    if function_name == "click_node":
        return {"success": bool(result.get("success")), "verified": bool(result.get("verified")), "selector": result.get("selector"), "message": result.get("message", ""), "error": result.get("error")}
    if function_name == "find_android_app":
        return {"success": bool(result.get("success")), "verified": bool(result.get("verified")), "packages": result.get("packages", []), "message": result.get("message", "")}
    return result


def _tool_result_message(tool_call, result, function_name):
    planner_result = _planner_tool_result(result, function_name)
    if isinstance(result, dict) and "post_action_observation" in result and isinstance(planner_result, dict):
        planner_result["post_action_observation"] = result["post_action_observation"]
    return {"role": "tool", "tool_call_id": tool_call["id"], "content": json.dumps(planner_result, ensure_ascii=False, separators=(",", ":"))}
...
def _run_simple_open_goal(app_name):
    ...
    if verification.get("success") and foreground:
        ...
        if len(packages) == 1 and foreground == packages[0]:
            return {"success": True, "verified": True, "message": f"{app_name} is already open and in the foreground.", "steps": 0}
    discovery = _unwrap_tool_result(execute_tool("find_android_app", "find_android_app", app_name=app_name))
    packages = discovery.get("packages") or [] if isinstance(discovery, dict) else []
    if not packages:
        return {"success": False, "verified": bool(discovery.get("verified")) if isinstance(discovery, dict) else False, "message": f"I couldn't find an installed app matching '{app_name}'.", "steps": 0}
    if len(packages) != 1:
        return {"success": False, "verified": bool(discovery.get("verified")) if isinstance(discovery, dict) else False, "message": f"The installed-app identity for '{app_name}' is ambiguous; Nova will not guess.", "steps": 0}
    package = packages[0]
    launch = _unwrap_tool_result(execute_tool("launch_android_app", "launch_android_app", package=package))
    if not launch.get("success"):
        return {"success": False, "verified": False, "message": launch.get("message", "The app could not be launched."), "steps": 0}
    for attempt in range(1, SIMPLE_OPEN_VERIFY_ATTEMPTS + 1):
        ...
        if verification.get("success") and foreground == package:
            return {"success": True, "verified": True, "message": f"{app_name} is open and verified in the foreground.", "steps": 0}
    return {"success": False, "verified": False, "message": f"I launched {app_name}, but after waiting for the app to become foreground, verification still shows {foreground or 'another app'}.", "steps": 0}
...
def _run_simple_open_path_goal(targets):
    ...
    return {"success": result.success, "verified": result.verified, "message": result.message, "steps": len(result.completed_targets), "targets": result.targets, "completed_targets": result.completed_targets, "failed_target": result.failed_target, "checkpoints": result.checkpoints}
...
def run_agent(goal):
    ...
    messages = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": "Accomplish this goal on the Android phone:\n" + goal}]
    action_seen = False
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if not tool_calls:
            ...
            if action_seen and not observed_after_action:
                messages.append({"role": "system", "content": "A state-changing action occurred but the resulting state has not been observed. Call observe_android before claiming success."})
                continue
        ...
        if result is None:
            if function_name not in AGENT_TOOLS:
                ...
                print("⚙️", result)
                if function_name not in {"observe_android", "find_android_app"} and isinstance(result, dict) and result.get("success"):
                    fresh_observation = _observe_directly()
                    if isinstance(fresh_observation, dict):
                        print("👀 Auto-observe:", _planner_tool_result(fresh_observation, "observe_android"))
    ...
    return {"success": False, "verified": False, "message": "Nova reached its maximum planning steps before completing the goal.", "steps": MAX_STEPS}
