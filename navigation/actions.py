...
def _accessibility_broadcast(action: str, *, target: str = "", direction: str = "") -> Tuple[bool, str, int, float]:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=(ACCESSIBILITY_SCROLL_TIMEOUT_SECONDS if "SCROLL" in action else ACCESSIBILITY_CLICK_TIMEOUT_SECONDS),
            check=False,
        )
...
def activate_node(node: Optional[Dict[str, Any]]) -> ActionResult:
    """Activate a live semantic node through Accessibility only.

    Root input is deliberately not a click fallback. A rejected semantic
    activation remains a failed action until bounded higher-level recovery
    explicitly re-observes and selects another strategy.
    """
    ...
    return ActionResult(
        False,
        "TAP",
        "Accessibility Service receiver rejected the requested action (result=0); no root fallback was attempted.",
        bounds,
        duration_ms,
        returncode,
        "Accessibility Service receiver rejected the requested action (result=0); no root fallback was attempted.",
...
def scroll(snapshot, direction: str, *, distance_ratio: float = 0.35) -> ActionResult:
    ...
    root_success, root_message, root_returncode, root_duration_ms = _root_input(f"input swipe {x} {start_y} {x} {end_y} 280")
    if root_success:
